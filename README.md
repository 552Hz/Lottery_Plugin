...
Windows Server 如果生成图片里的中文变成方框或乱码，请确认 AstrBot 实际运行的插件目录里包含 `NotoSansSC-Regular.otf`，并且它和 `fortune_card.py` 在同一层。本仓库已经自带这个开源中文字体，插件会优先加载它。

更新代码后请重启 AstrBot。新版缓存文件名会带 `_font4`，如果生成的图片文件名不是 `_font4`，说明 AstrBot 还在运行旧代码或旧缓存。
